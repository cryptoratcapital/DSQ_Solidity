...
// Same code but airdrop accepts random number parameter & extra function with console log of winner
contract TestFixture_Airdrop is Ownable {
    // ----- Events -----
    ...
    function airdrop(uint256 rand) external onlyOwner {
    ...
    function test_runAgain() external onlyOwner {
        uint256 endSale = privateContribution.endTime();
        require(block.timestamp > endSale, "!ready");
        ...
        // Select winner
        require(entries > 0, "!entries");
        uint256 rand = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, block.number))) % lastDayContributionTotal;
        for (uint256 i = contributions.length - entries; i < contributions.length; ++i) {
            if (rand < contributions[i].amount) {
                winner = contributions[i].contributer;
                console.log(winner);
