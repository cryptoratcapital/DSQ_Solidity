...
describe("Vault V0", function () {
  beforeEach(async function () {
    ...
    Vault = await ethers.getContractFactory("VaultV0");
    vault = await Vault.deploy(USDC.address, "USDC Vault", "DSQ-USDC-V0", trader.address, MAX_DEPOSITS);
    ...
    await vault.connect(devWallet).setWhitelistStatuses([citizen1.address, citizen2.address], [true, true]);
  ...
  it("Should NOT start an epoch with improper timing parameters", async function () {
    ...
    await expect(vault.connect(devWallet).startEpoch(fundingStart + 10, epochStart, epochStart + 61 * 24 * 3600)).to.be.revertedWith(
      "!epochLen",
    );
  });
  ...
  it("Should deposit/mint only if whitelisted", async function () {
    await vault.connect(devWallet).startEpoch(fundingStart + 100, epochStart, epochEnd);
    ...
    expect(await vault.connect(citizen1).deposit(0.5e9, citizen1.address))
      .to.emit(vault, "Deposit")
      .withArgs(citizen1.address, citizen1.address, 0.5e9, 0.5e9);

    expect(await vault.connect(citizen2).mint(0.5e9, citizen2.address))
      .to.emit(vault, "Deposit")
      .withArgs(citizen2.address, citizen2.address, 0.5e9, 0.5e9);

    expect(await vault.connect(devWallet).setWhitelistStatuses([citizen1.address], [false]))
      .to.emit(vault, "NewWhitelistStatus")
      .withArgs(citizen1.address, false);
    expect(await vault.connect(devWallet).setWhitelistStatus(citizen2.address, false))
      .to.emit(vault, "NewWhitelistStatus")
      .withArgs(citizen2.address, false);

    await expect(vault.connect(citizen1).deposit(0.5e9, citizen1.address)).to.be.revertedWith("!whitelisted");

    await expect(vault.connect(citizen1).mint(0.5e9, citizen1.address)).to.be.revertedWith("!whitelisted");
  });
  ...
  it("Should show isFunding and isInEpoch", async function () {
    await vault.connect(devWallet).startEpoch(fundingStart, epochStart, epochEnd);

    expect(await vault.isInEpoch()).to.eq(false);
    expect(await vault.isFunding()).to.eq(false);
    ...
    expect(await vault.isInEpoch()).to.eq(false);
    expect(await vault.isFunding()).to.eq(true);

    await network.provider.send("evm_setNextBlockTimestamp", [epochStart]);
    await network.provider.send("evm_mine");

    expect(await vault.isInEpoch()).to.eq(true);
    expect(await vault.isFunding()).to.eq(false);
  ...
  it("Should split profits with two equal deposits", async function () {
    ...
    expect(await vault.connect(citizen1).previewRedeem(1e9)).to.eq(2e9);
    expect(await vault.connect(citizen1).previewWithdraw(2e9)).to.eq(1e9);
  ...
  it("previewDeposit should return correct amount of shares if assets double", async function () {
    await vault.connect(devWallet).startEpoch(fundingStart, epochStart, epochEnd);
  ...
  it("previewDeposit should return correct amount of shares if assets halve", async function () {
    await vault.connect(devWallet).startEpoch(fundingStart, epochStart, epochEnd);
