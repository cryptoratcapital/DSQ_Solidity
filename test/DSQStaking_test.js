...
xdescribe("DSQStaking", function () {
  ...
  describe("Staking", function () {
    ...
    it("Should NOT getReward if no reward", async function () {
      await expect(router.connect(citizen1).harvestDSQStaking()).to.be.revertedWith("DSQStaking: No reward");
    });
  ...
  describe("Admin", function () {
    ...
    it("Should set rewards duration", async function () {
      ...
      await expect(dsqStaking.connect(citizen1).setRewardsDuration(69)).to.be.reverted;
    });
    ...
    it("Should set rewards distribution", async function () {
      ...
      await expect(dsqStaking.connect(citizen1).setRewardsDistribution(citizen2.address)).to.be.reverted;
    });
    ...
    it("Should setRouter", async function () {
      await expect(dsqStaking.connect(multisig).setRouter(router.address))
        .to.emit(dsqStaking, "NewRouter")
