...
async function main() {
  ...
  // Deploy the contract
  const LyraLPFacet = await hre.ethers.getContractFactory("Lyra_LP_Module");
  const lyraLPFacet = await LyraLPFacet.deploy(addresses.USDC);
  await lyraLPFacet.deployed();
  console.log("Lyra LP facet deployed to: ", lyraLPFacet.address);

  // Write address and constructor arguments to json file
  let deploymentInfo = {
    address: lyraLPFacet.address,
    constructorArguments: [addresses.USDC],
  };
  const FILENAME = "LyraLPFacetDeployment.json";
  try {
  ...
  await lyraLPFacet.deployTransaction.wait(txConfirmations);
...
// npx hardhat run scripts/deploy/trader/facets/lyra/Lyra_LP_Facet.js --network arbitrum
