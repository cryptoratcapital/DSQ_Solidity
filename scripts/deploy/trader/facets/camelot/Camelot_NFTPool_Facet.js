...
async function main() {
  ...
  // Deploy the contract
  const CamelotNFTPoolFacet = await hre.ethers.getContractFactory("Camelot_NFTPool_Module");
  const camelotNFTPoolFacet = await CamelotNFTPoolFacet.deploy();
  await camelotNFTPoolFacet.deployed();
  console.log("Camelot NFT Pool facet deployed to: ", camelotNFTPoolFacet.address);

  // Write address and constructor arguments to json file
  let deploymentInfo = {
    address: camelotNFTPoolFacet.address,
    constructorArguments: [],
  };
  const FILENAME = "CamelotNFTPoolFacetDeployment.json";
  try {
  ...
  await camelotNFTPoolFacet.deployTransaction.wait(txConfirmations);
...
// npx hardhat run scripts/deploy/trader/facets/camelot/Camelot_NFTPool_Facet.js --network arbitrum
