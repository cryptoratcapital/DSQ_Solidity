...
async function main() {
  ...
  // Deploy the contract
  const Strategy = await hre.ethers.getContractFactory("Strategy_ARB");
  const strategy = await Strategy.deploy(DSQ_MULTISIG, TRADER_FACET, traderInitializerParams, FACETS, assets, oracles);
  await strategy.deployed();
  console.log("Strategy_ARB deployed to: ", strategy.address);
  ...
  const FILENAME = "StrategyARBDeployment.json";
  try {
...
// npx hardhat run scripts/deploy/trader/strategies/Strategy_ARB.js --network arbitrum
