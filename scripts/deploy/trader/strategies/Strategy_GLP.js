...
async function main() {
  ...
  // Deploy the contract
  const Strategy = await hre.ethers.getContractFactory("Strategy_GLP");
  const strategy = await Strategy.deploy(DSQ_MULTISIG, TRADER_FACET, traderInitializerParams, FACETS, assets, oracles);
  await strategy.deployed();
  console.log("Strategy_GLP deployed to: ", strategy.address);
  ...
  const FILENAME = "StrategyGLPDeployment.json";
  try {
...
// npx hardhat run scripts/deploy/trader/strategies/Strategy_GLP.js --network arbitrum
