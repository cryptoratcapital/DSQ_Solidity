...
async function main() {
  ...
  const dsqStaking = await DSQStaking.deploy(DSQ_MULTISIG, DSQ_MULTISIG, ESDSQ_ADDRESS, DSQ_ADDRESS);
  await dsqStaking.deployed();
  console.log("Contract deployed to: ", dsqStaking.address);

  // Write address and constructor arguments to json file
  let deploymentInfo = {
    address: dsqStaking.address,
    constructorArguments: [DSQ_MULTISIG, DSQ_MULTISIG, ESDSQ_ADDRESS, DSQ_ADDRESS],
  };
  const FILENAME = "DSQStakingDeployment.json";
  try {
  ...
  await dsqStaking.deployTransaction.wait(txConfirmations);
